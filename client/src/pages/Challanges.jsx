...
export default function Challanges() {
  ...
  const { challanges, filteredChallanges, setFilteredChallanges,setChallanges } =
    useChallanges();

  const { userSavedChallanges, setUserSavedChallanges } =
    useUserSavedChallanges(id);
  const { userJoinedchallanges,setUserJoinedChallanges } = useUserJoinedChallanges(id);
      ...
      <div className="max-w-6xl mx-auto p-6 mt-20">
        {email && (
          <div className="flex items-center justify-center bg-gray-900 text-white px-6 py-3 rounded-lg shadow-md mb-10">
            <Link className="text-lg font-semibold text-gray-300">
              Challenges Joined:
            </Link>
            <span className="ml-3 text-2xl font-extrabold text-orange-400">
              {userJoinedchallanges}
            </span>
            <Link className="text-lg font-semibold text-gray-300 ml-5">
              Challenges Saved:
            </Link>
            <span className="ml-3 text-2xl font-extrabold text-orange-400">
              {userSavedChallanges}
          ...
          <input
            ...
            onChange={(event) => {
              ...
              setFilteredChallanges([
                ...challanges.filter((c) =>
                  c.name.startsWith(event.target.value)
                ),
              ]);
            }}
          />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {filteredChallanges.map((challange) => (
            <ChallangeCard
              key={challange._id}
              challenge={challange}
              setUserSavedChallanges={setUserSavedChallanges}
              setUserJoinedChallanges = {setUserJoinedChallanges}
            />
          ))}
        </div>
      </div>
