...
import {
  ...
  useDeleteChallange,
  useJoinChallange,
  useUserJoinedChallanges,
} from "../../../api/challangeApi";
...
export default function ChallangeDetails() {
  ...
  const {setChallanges} = useChallanges()
  const { challange, setIsParticipants, isParticipants } = useChallange(id);


  const { deleteChallange, showModal, cancel, setShowModal } =
    useDeleteChallange(id);

  const { joinChallange, showJoinForm, setShowJoinForm, toggleJoin } =
    useJoinChallange(id);

  const { participates, setParticipants } = useParticipates(id);

  const isJoin =
    participates.filter((p) => p.userId?._id === currentUserId).length === 0
      ? false
      : true;

    const deleteChallangeHanlder = () =>{
      deleteChallange(id);
      setChallanges(oldState =>[...oldState.filter(c => c._id !== id)]);
    }  
  ...
  return (
    <>
      {showModal && (
        <IsSureModal
          cancel={cancel}
          deleteFunc={deleteChallangeHanlder}
              ...
              <div className="w-full md:w-2/3 md:pl-6">
                ...
                <div className="space-y-4">
                  ...
                  <p>
                    ...
                    {challange.duration} days
                  </p>
                  <p>
                    <strong className="font-medium">Equipment:</strong>{" "}
                    {challange.equipment}
                  </p>
                </div>
                      ...
                      <>
                        {isJoin ? (
                          <p className="text-black">
                            You are already joined participant!
                          </p>
                        ) : (
                          <button
                            className="px-6 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition duration-300"
      ...
      {participates.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-96 bg-gray-100 rounded-lg shadow-md p-6">
          <h2 className="text-2xl font-bold text-gray-800 mb-4">
            No Participants Yet!
          </h2>
          <p className="text-gray-600 text-center mb-6">
            Be the first to join this challenge and inspire others to
            participate!
          </p>
          {currentUserId ? (
            <button
        ...
        <div className="bg-gray-100 rounded-lg shadow-md p-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {[...participates.map((p) => p.userId)]
              .slice(-3)
              .map((participate) => (
                <Link to={`/fitzone/profile/${participate._id}`} key={participate._id}>
                  <img
                    key={participate._id}
      ...
      {isParticipants && <ChallangeParticipants participates={participates} />}
